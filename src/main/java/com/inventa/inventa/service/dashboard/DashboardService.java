...
public class DashboardService {
    ...
    public ResumenDashboardDTO getResumenDashboard() {
        String sql = "SELECT " +
            "COALESCE((SELECT COUNT(*) FROM tienda_garcia.producto), 0) AS totalProductos, " +
            "COALESCE((SELECT SUM(stock_actual) FROM tienda_garcia.producto), 0) AS stockTotal, " +
            "COALESCE((SELECT SUM(stock_actual * ultimo_costo) FROM tienda_garcia.producto), 0) AS valorInventario, " +
            "COALESCE((SELECT SUM(monto_total) FROM tienda_garcia.venta), 0) AS totalVentasHistorico, " +
            "COALESCE((SELECT SUM(dv.cantidad * (dv.precio_unitario_venta - p.ultimo_costo)) FROM tienda_garcia.detalle_venta dv JOIN tienda_garcia.lote l ON dv.lote_id = l.lote_id JOIN tienda_garcia.producto p ON l.producto_id = p.producto_id), 0) AS gananciaBrutaHistorica, " +
            "COALESCE((SELECT COUNT(*) FROM tienda_garcia.cliente), 0) AS totalClientes";
    ...
    public AlertasDashboardDTO getAlertasDashboard() {
        String stockSql = "SELECT producto_id, nombre, sku, stock_actual, stock_minimo FROM tienda_garcia.producto WHERE stock_actual <= stock_minimo ORDER BY stock_actual ASC LIMIT 10";
        ...
        String caducidadSql = "SELECT l.lote_id, p.nombre, l.fecha_caducidad, l.cantidad_actual, p.sku FROM tienda_garcia.lote l JOIN tienda_garcia.producto p ON l.producto_id = p.producto_id WHERE l.fecha_caducidad BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '30 day' AND l.cantidad_actual > 0 ORDER BY l.fecha_caducidad ASC LIMIT 10";
    ...
    public MovimientosRecientesDTO getMovimientosRecientes() {
        String entradasSql = "(SELECT c.fecha_compra AS fecha, 'Compra' AS tipo, dc.cantidad, p.nombre AS producto_nombre, u.nombre_usuario AS usuario FROM tienda_garcia.detalle_compra dc JOIN tienda_garcia.compra c ON dc.compra_id = c.compra_id JOIN tienda_garcia.producto p ON dc.producto_id = p.producto_id JOIN tienda_garcia.usuario u ON c.usuario_id = u.usuario_id ORDER BY c.fecha_compra DESC LIMIT 5) " +
                             "UNION ALL " +
                             "(SELECT ai.fecha_ajuste::date AS fecha, 'Ajuste Entrada' AS tipo, ai.cantidad, p.nombre AS producto_nombre, u.nombre_usuario AS usuario FROM tienda_garcia.ajuste_inventario ai JOIN tienda_garcia.lote l ON ai.lote_id = l.lote_id JOIN tienda_garcia.producto p ON l.producto_id = p.producto_id JOIN tienda_garcia.usuario u ON ai.usuario_id = u.usuario_id WHERE ai.tipo_ajuste = 'Entrada' ORDER BY ai.fecha_ajuste DESC LIMIT 5) ORDER BY fecha DESC LIMIT 5";
        ...
        String salidasSql = "(SELECT v.fecha_venta::date AS fecha, 'Venta' AS tipo, dv.cantidad, p.nombre AS producto_nombre, u.nombre_usuario AS usuario FROM tienda_garcia.detalle_venta dv JOIN tienda_garcia.venta v ON dv.venta_id = v.venta_id JOIN tienda_garcia.lote l ON dv.lote_id = l.lote_id JOIN tienda_garcia.producto p ON l.producto_id = p.producto_id JOIN tienda_garcia.usuario u ON v.usuario_id = u.usuario_id ORDER BY v.fecha_venta DESC LIMIT 5) " +
                            "UNION ALL " +
                            "(SELECT ai.fecha_ajuste::date AS fecha, 'Ajuste Salida' AS tipo, ai.cantidad, p.nombre AS producto_nombre, u.nombre_usuario AS usuario FROM tienda_garcia.ajuste_inventario ai JOIN tienda_garcia.lote l ON ai.lote_id = l.lote_id JOIN tienda_garcia.producto p ON l.producto_id = p.producto_id JOIN tienda_garcia.usuario u ON ai.usuario_id = u.usuario_id WHERE ai.tipo_ajuste = 'Salida' ORDER BY ai.fecha_ajuste DESC LIMIT 5) ORDER BY fecha DESC LIMIT 5";
    ...
    public List<TopProductoDashboardDTO> getTopProductosMasVendidos() {
        String sql = "SELECT p.producto_id, p.nombre, p.sku, SUM(dv.cantidad) AS totalUnidadesVendidas, SUM(dv.subtotal) AS ingresosGenerados " +
                     "FROM tienda_garcia.detalle_venta dv " +
                     "JOIN tienda_garcia.lote l ON dv.lote_id = l.lote_id " +
                     "JOIN tienda_garcia.producto p ON l.producto_id = p.producto_id " +
    ...
    public List<ValorCategoriaDTO> getValorInventarioPorCategoria() {
        String sql = "SELECT cat.nombre AS categoria, SUM(p.stock_actual * p.ultimo_costo) AS valorInventario, COUNT(p.producto_id) AS totalProductos " +
                     ...
                     "JOIN tienda_garcia.categoria cat ON p.categoria_id = cat.categoria_id " +
